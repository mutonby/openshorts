...
def _resolve_font_dir() -> str:
    """Locate the committed font dir whether running from repo root or backend/."""
    env_override = os.environ.get("OPENSHORTS_FONT_DIR")
    if env_override:
        return env_override
    # Walk up from this file (backend/app/overlays/hooks.py) looking for assets/fonts.
    here = Path(__file__).resolve()
    for parent in here.parents:
        candidate = parent / "assets" / "fonts"
        if candidate.is_dir():
            return str(candidate)
    # Last resort: cwd-relative (used as the download target).
    return "assets/fonts"
