...
def create_hook_image(
    text, target_width, output_image_path="hook_overlay.png", font_scale=1.0
):
    """
    ...
    draw_final.rounded_rectangle(
        main_box, radius=cornerradius, fill=(255, 255, 255, 240)
    )
...
def add_hook_to_video(video_path, text, output_path, position="top", font_scale=1.0):
    ...
    try:
        ...
        elif position == "above_blur":
            # Just above the centered video (blur bar area)
            overlay_y = int(video_height * 0.28)
        elif position == "below_blur":
            # Just below the centered video (blur bar area)
            overlay_y = int(video_height * 0.72)
        else:
            # Top 20% mark (default)
